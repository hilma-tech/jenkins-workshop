...
function App() {
  ...
  const getAnswer = async () => {
    try {
      const res = await axios.post("http://localhost:8109/api/get-answer", { myPassword })
      setData(res.data)
    } catch (error) {
      ...
      <body >
        <div onClick={() =>{if (error) setError(null)}} style={{ filter: error ? "blur(5px) brightness(-20%)" : undefined }}>
          ...
          <div style={{ margin: "0.7rem", display: "flex", flexDirection: "row", gap: "0.5rem" }}>
            <input type="password" value={myPassword} onChange={e => { setMyPassword(e.target.value) }} style={{ width: "13rem", height: "1.5rem" }} placeholder="type it here dont be shy"></input>
